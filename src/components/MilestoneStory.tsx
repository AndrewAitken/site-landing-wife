...
export function MilestoneStory({ year, age, title, description, quote, imageUrl }: MilestoneStoryProps) {
  return (
        ...
        <div className="overflow-clip rounded-[inherit] size-full">
          <div className="box-border content-stretch flex flex-col h-[320px] items-start justify-between p-[24px] relative w-full">
            <div className="backdrop-blur-[2px] backdrop-filter bg-[rgba(255,255,255,0.8)] box-border content-stretch flex gap-[12px] items-center px-[14px] py-[5px] relative rounded-[1.75866e+07px] shrink-0">
              <p className="font-normal leading-[24px] relative shrink-0 text-[16px] text-black text-nowrap tracking-[-0.3125px] whitespace-pre">{year} год</p>
            </div>
            <p className="font-normal leading-[48px] relative shrink-0 text-[40px] text-nowrap text-white tracking-[0.2637px] whitespace-pre">{age}</p>
          </div>
        </div>
      </div>
          ...
          <div className="box-border content-stretch flex flex-col gap-[16px] items-start p-[24px] relative w-full">
            <div className="content-stretch flex flex-col gap-[11.998px] items-start relative shrink-0 w-full">
              <div className="h-[24.003px] relative shrink-0 w-full">
                <p className="absolute font-medium leading-[24px] left-0 text-[18px] text-black text-nowrap top-[-0.9px] tracking-[-0.3125px] whitespace-pre">{title}</p>
              </div>
              <p className="font-normal leading-[22.75px] relative shrink-0 text-[#4a5565] text-[14px] tracking-[-0.1504px] w-full">{description}</p>
            </div>
            <div className="box-border content-stretch flex flex-col items-start pb-0 pt-[16.518px] px-0 relative shrink-0 w-full">
              <div aria-hidden="true" className="absolute border-[0.524px_0px_0px] border-gray-200 border-solid inset-0 pointer-events-none" />
              <p className="font-normal italic leading-[16px] relative shrink-0 text-[#6a7282] text-[12px] w-full">"{quote}"</p>
            </div>
          </div>
        </div>
      </div>
